...
class AnomalyDetection:
    ...
    def preprocess_data(self, df, model_name):
        ...
        # Ajout de colonnes spécifiques aux modèles
        if model_name in ['6082', '6084']:
            df['6084Base'] = df['6084Base'].fillna(0)
            df['1001Montant Sal.'] = df['Assiette Mois M (/102)'] - df.get('6S89Base', 0).fillna(0)

        # Vérification des colonnes nécessaires
        required_columns = []
        if "cases" in config:
            for case in config["cases"].values():
                required_columns.extend([case["feature_col"], case["target_col"]])
        missing_columns = [col for col in required_columns if col not in df.columns]
        if missing_columns:
            raise ValueError(f"Colonnes manquantes pour {model_name} : {missing_columns}")
        def determine_case(row):
            if row['Brut CUM'] > 4*row['Plafond CUM']:
                return 'Cas 1'
            else:
                return 'Cas 2'
        ...
        if 'Brut CUM' in df.columns and 'Plafond CUM' in df.columns:
            df['Case'] = df.apply(determine_case, axis=1)
        else:
            raise ValueError(f"Colonnes manquantes pour déterminer la colonne 'Case' dans {model_name}")
        
        # Vérifier la création de la colonne
        if 'Case' not in df.columns:
            raise ValueError(f"La colonne 'Case' n'a pas été créée pour le modèle {model_name}")
        
        if model_name == '7025':

            
            df['SMIC CUM'] = df['SMIC M CUM M-1'].fillna(0) + df['SMIC M'].fillna(0)  # Ajout de .fillna(0) pour éviter les NaN
            df['Plafond CUM'] = round(3.5 * df['SMIC CUM'], 2)
            df['7025Base'] = df['7025Base'].fillna(0)
            df['anomalie'] = 0  # Initialisation de la colonne 'anomalie'

            # Nettoyage et renommage des colonnes
            df['Total Brut'] = df['Total Brut'].replace({'\.': '', ',': '.'}, regex=True).astype(float)
            df.rename(columns={
                "Cumul d'assiette ( Mois courant inclus) (/102)": 'Brut CUM',
                "BASE B 7025": 'Base CUM M-1'
            }, inplace=True)
            df['Base CUM M-1'] = df['Base CUM M-1'].fillna(0)
            df['Cluster'] = df.apply(lambda row: "Cas 1.1" if row['Brut CUM'] < row['Plafond CUM'] and row['Base CUM M-1'] == 0 else "Cas 2.2", axis=1)

            def creer_labels(row):
                """
                Déterminer le cluster en fonction des règles métier sur 'Brut CUM' et 'Base CUM M-1'.
                """
                # Vérifications pour éviter des erreurs dues à des valeurs manquantes ou incohérentes
                if pd.isnull(row['Brut CUM']) or pd.isnull(row['Plafond CUM']) or pd.isnull(row['Base CUM M-1']):
                    return 'Données Incomplètes'

                # Appliquer la logique métier avec des comparaisons robustes
                if row['Brut CUM'] < row['Plafond CUM']:
                    if row['Base CUM M-1'] == 0:
                        return 'Cas 1.1'
                    else:
                        return 'Cas 1.2'
                else:
            ...
            def ajouter_labels(donnees):
                """
                Ajouter une colonne 'Cluster' en appliquant la logique métier définie dans 'creer_labels'.
                Vérifie que les données nécessaires sont disponibles.
                """
                # Vérifier que toutes les colonnes nécessaires existent et ne contiennent pas uniquement des valeurs nulles
                required_columns = ['Brut CUM', 'Plafond CUM', 'Base CUM M-1']
                missing_or_null = [col for col in required_columns if col not in donnees.columns or donnees[col].isnull().all()]
                if missing_or_null:
                    raise ValueError(f"Les colonnes suivantes sont manquantes ou complètement nulles : {missing_or_null}")

                # Vérifier si le DataFrame est vide avant d'appliquer la logique
                if donnees.empty:
                    raise ValueError("Le DataFrame est vide. Impossible d'ajouter les labels.")

                # Appliquer la fonction pour créer les labels
                donnees['Cluster'] = donnees.apply(lambda row: creer_labels(row), axis=1)
                return donnees


            # Appel de la fonction pour ajout des labels
            try:
                df = ajouter_labels(df)
